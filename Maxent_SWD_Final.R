...
data <- read.csv("OccurenceData/MasterPycno_2021-03-03_SAG.csv")
...
setwd("C:/Users/hamiltsa/Desktop/Pycnopodia/Manuscript/FinalDataandScripts/Maxent_SWD")
pycnos <- read.csv("OccurenceData/MasterPycno_2021-03-03_SAG.csv", stringsAsFactors = FALSE)
pre <- pycnos %>%
...
depth <-raster("EnvironmentalFiles/Pre/Depth.asc")
...
occ <- read.csv("C:/Users/hamiltsa/Desktop/Pycnopodia/SDM/Data/Pycnopodia/Presence_20172020_Albers.csv")
occ <- occ[,-1]
##Extract in this order: 1) depth 2) 90th percentile of temps 3) mean Chl, 4) mean salinity and 5) substrate type 
#depth
depth <- raster("EnvironmentalFiles/Post/Depth.asc")
crs(depth) <- Albers2
occ_var <- raster::extract(depth, occ[,2:3])
occ_swd <- cbind(occ, occ_var)
colnames(occ_swd) <- c("Species","Longitude","Latitude","Depth")
#temp
var <- raster("EnvironmentalFiles/Post/90th_%_Temp.asc")
occ_var <- raster::extract(var, occ_swd[,2:3])
occ_swd <- cbind(occ_swd, occ_var)
colnames(occ_swd) <- c("Species","Longitude","Latitude","Depth","90th_%_Temp")
#chl
var <- raster("EnvironmentalFiles/Post/Mean_Chl.asc")  
crs(var) <- Albers2  
occ_var <- raster::extract(var, occ[,2:3])
occ_swd <- cbind(occ_swd, occ_var)
colnames(occ_swd) <- c("Species","Longitude","Latitude","Depth","90th_%_Temp", "Mean_Chl")
#sal
var <- raster("EnvironmentalFiles/Post/Salinity.asc")
occ_var <- raster::extract(var, occ[,2:3])
occ_swd <- cbind(occ_swd, occ_var)
colnames(occ_swd) <- c("Species","Longitude","Latitude","Depth","90th_%_Temp", "Mean_Chl", "Salinity")
#Substrate type
var <- raster("EnvironmentalFiles/Post/Substrate.asc" )
occ_var <- raster::extract(var, occ_swd[,2:3])
occ_swd <- cbind(occ_swd, occ_var)
colnames(occ_swd) <- c("Species","Longitude","Latitude","Depth","90th_%_Temp", "Mean_Chl", "Salinity","Substrate")
#Done!
...
#depth
var <- raster("EnvironmentalFiles/Post/Depth.asc")
crs(var) <- Albers2
occ_var <- raster::extract(var, occ[,2:3])
occ_swd <- cbind(occ, occ_var)
colnames(occ_swd) <- c("Species","Longitude","Latitude","Depth")
...
pre <- read.csv("Pre_Log_LQP_RM1_Tau061/Percent_Importance_Pre.csv")
post <- read.csv("Post_Log_LQP_RM1_Tau014/Percent_Importance_Post.csv")
